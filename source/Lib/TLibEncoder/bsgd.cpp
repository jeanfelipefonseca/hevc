...
float predictBSGD(budgetedData *testData, parameters *param, budgetedModelBSGD *model, vector <char> *labels)
{
	...
	while (stillChunksLeft)
	{
    	...
    	for (unsigned int r = 0; r < N; r++)
    	{
    		...
    		for (unsigned int k2 = 0; k2 < (testData->yLabels).size(); k2++)
    		{
				fx = 0;
				for (unsigned int i2 = 0; i2 < (*(model->modelBSGD)).size(); i2++)
				{			
					if ((*(model->modelBSGD))[i2]->alphas[k2] != 0)
					{
						if (tempArray[i2] == 0.0)
						{
							if ((*param).VERY_SPARSE_DATA)
								// directly compute kernel from the trainData
								tempArray[i2] = (*((*model).modelBSGD))[i2]->gaussianKernel(r, testData, param, tempSqrNorm);
							else
								// compute kernel from currentData object
								tempArray[i2] = (*(model->modelBSGD))[i2]->gaussianKernel(currentData, param);
						}
						fx += ((*(model->modelBSGD))[i2]->alphas[k2] * tempArray[i2]);
					}
				}
	...
	for (unsigned int epoch = 0; epoch < (*param).NUM_EPOCHS; epoch++)
	{
		...
		while (stillChunksLeft)
		{
			...
			else if (numClasses != (unsigned int) trainData->yLabels.size())
			{
				...
				for (unsigned int i3 = 0; i3 < numSVs; i3++)
					for (unsigned int k2 = 0; k2 < (trainData->yLabels.size() - numClasses); k2++)
						(*((*model).modelBSGD))[i3]->alphas.push_back(0.0);
				numClasses = (unsigned int) trainData->yLabels.size();
			}
					...
					{						
						switch ((*param).MAINTENANCE_SAMPLING_STRATEGY)
						{
							...
							case 1:								
								...
								for (unsigned int k3 = 0; k3 < numClasses; k3++)
									(*((*model).modelBSGD))[merge1]->alphas[k3] = (*((*model).modelBSGD))[merge1]->alphas[k3] * kZ1 + (*((*model).modelBSGD))[merge2]->alphas[k3] * kZ2;
